...
function getThings(): Promise<object[]> {
  //  Returns a promise for a list of things for the current product.
  return new Promise((resolve, reject) =>
    thethingsAPI.getProductThings((error, result) =>
...
function writeThing(thingToken: string, values: {key: string, value: any}[]): Promise<any> {
  //  Write the array of values {key, value} in values to the thing state.  Returns a promise.
  return new Promise((resolve, reject) =>
    thethingsAPI.thingWrite(thingToken, { values }, (error, result) =>
      (error ? reject(error) : resolve(result)))
  )
    .then(result => {
  ...
  return getThings()
    ...
    .then(() => {
      ...
      alerts.forEach(alert => {
        ...
        const thing = allThings.find(thg =>
          (thg.tags && thg.tags[0] && thg.tags[0].name == instance));  //  TODO: Allow more than 1 tag
        if (!thing) return;
